...
resource "aws_route53_record" "zx-cloud" {
  for_each = {
    for dvo in aws_acm_certificate.zx-cloud.domain_validation_options : dvo.domain_name => {
...
resource "aws_acm_certificate_validation" "zx-cloud" {
  ...
  validation_record_fqdns = [for record in aws_route53_record.zx-cloud : record.fqdn]
}

# create records for tooling
resource "aws_route53_record" "tooling" {
  zone_id = data.aws_route53_zone.zx-cloud.zone_id
  name    = "tooling.zx-cloud.uk"
  type    = "A"
